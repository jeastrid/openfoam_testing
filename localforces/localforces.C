...
{
    defineTypeNameAndDebug(localforces, 0);
    addToRunTimeSelectionTable(functionObject, localforces, dictionary);
}
}
...
void Foam::functionObjects::localforces::setCoordinateSystem
(
...
Foam::volVectorField& Foam::functionObjects::localforces::force()
{
...
Foam::volVectorField& Foam::functionObjects::localforces::moment()
{
...
void Foam::functionObjects::localforces::initialise()
{
    if (initialised_)
    {
        return;
...
void Foam::functionObjects::localforces::reset()
{
...
Foam::functionObjects::localforces::devRhoReff
(
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::localforces::mu() const
{
...
Foam::tmp<Foam::volScalarField> Foam::functionObjects::localforces::rho() const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::functionObjects::localforces::rho(const label patchi) const
{
    if (rhoName_ == "rhoInf")
    {
...
Foam::scalar Foam::functionObjects::localforces::rho(const volScalarField& p) const
{
...
void Foam::functionObjects::localforces::addToPatchFields
(
    const label patchi,
...
void Foam::functionObjects::localforces::addToInternalField
(
...
void Foam::functionObjects::localforces::createIntegratedDataFiles()
{
    if (!forceFilePtr_)
    {
...
void Foam::functionObjects::localforces::writeIntegratedDataFileHeader
(
    const word& header,
    OFstream& os
) const
...
void Foam::functionObjects::localforces::writeIntegratedDataFiles()
{
    const auto& coordSys = coordSysPtr_();
...
void Foam::functionObjects::localforces::writeIntegratedDataFile
(
    const vector& pres,
    const vector& vis,
...
void Foam::functionObjects::localforces::logIntegratedData
(
...
Foam::functionObjects::localforces::localforces
(
    const word& name,
    const Time& runTime,
    const dictionary& dict,
...
bool Foam::functionObjects::localforces::read(const dictionary& dict)
{
...
void Foam::functionObjects::localforces::calcForcesMoments()
{
    initialise();
    ...
    // R               [m] circle radius
    // xCB             [m] location of CB in inertial frame at start
    // radialVelocity  [rad/s] circular frequency
    // x               [m] x-coordinate of circular trajectory
    // y               [m] y-coordinate of circular trajectory
    // a               [rad] angular position of coordinate system from CofR;
    //                 the +piByTwo makes reference to positive x-axis
    const Time& time = mesh_.time();
    const scalar R = 20;
    const scalar xCB = 0;
    const scalar radialVelocity = 22.5 * constant::mathematical::pi / 180;
    const scalar x = xCB - R * sin(radialVelocity  * time.value());
    const scalar y = R * cos(radialVelocity  * time.value()) - R;
    const scalar a = radialVelocity * time.value() + constant::mathematical::piByTwo;

    const vector e1 = vector(cos(a), sin(a), 0);
    const vector e3 = vector(0, 0, 1);
    const point origin = vector(x, y, 0);
    Log << "Local coordinate system position and orientation:" << nl;
    Log << "origin : " << origin << nl;
    Log << "e1     : " << e1 << nl << nl;
    coordSysPtr_.reset(new coordSystem::cartesian(origin, e3, e1));
...
Foam::vector Foam::functionObjects::localforces::forceEff() const
{
    return sumPatchForcesP_ + sumPatchForcesV_ + sumInternalForces_;
}


Foam::vector Foam::functionObjects::localforces::momentEff() const
{
    return sumPatchMomentsP_ + sumPatchMomentsV_ + sumInternalMoments_;
}


bool Foam::functionObjects::localforces::execute()
{
    calcForcesMoments();
    ...
    logIntegratedData("localforces", localFp, localFv, localFi);
...
bool Foam::functionObjects::localforces::write()
{
    if (writeToFile())
    {
